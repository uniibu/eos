...
function main() {
  return async function startClient() {
    ...
    if (!isExist) {
      ...
      logger.info('Clearing in', DEVELOPMENT ? 3 : 30, 'seconds');
      const key = await dataBase.get('secret.key').value();
      const privateLog = boxen(`Environment: ${process.env.NODE_ENV}
        Withdraw Url: http://${(await helpers.getPubIp()).trim()}:8866/withdraw?key=${key}
        Wallet Address: ${ HOTWALLET_ACCOUNT}
        Wallet Owner Private Key: ${await dataBase.get('owner.private').value()}
        Wallet Owner Public Key: ${await dataBase.get('owner.public').value()}
        Wallet Active Private Key: ${await dataBase.get('active.private').value()}
        Wallet Active Public Key: ${await dataBase.get('active.public').value()}
    ...
    const hotWalletPrivKey = await dataBase.get('active.private').value();
    if (!hotWalletPrivKey) {
